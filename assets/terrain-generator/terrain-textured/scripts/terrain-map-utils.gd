...
static func generate_mesh_old(noise_map : PackedFloat32Array, terrain_resolution : int, lod_scale : float, terrain_offset : Vector2, terrain_height : float, smooth_faces : bool) -> ArrayMesh:
	...
	for z in range(terrain_resolution + 1):
		for x in range(terrain_resolution + 1):
			...
			#var y = n.get_noise_2d(x * noise_offset, z * noise_offset) * terrain_height
			var y = noise_map[x + z * (terrain_resolution + 1)]
			...
			#	Set the UV coordinates
			var uv = Vector2()
			uv.x = inverse_lerp(0, terrain_resolution, x)
			uv.y = inverse_lerp(0, terrain_resolution, z)
			...
			#	Set the vertex coordinates
			var vertex_position = Vector3(x, y, z)
			vertex_position.x = vertex_position.x * lod_scale + terrain_offset.x
			vertex_position.y = vertex_position.y * terrain_height
			vertex_position.z = vertex_position.z * lod_scale + terrain_offset.y
	...
	#print(min * terrain_height)
	#print(max * terrain_height)
	
	var vert_idx = 0
	for z in terrain_resolution:
		for x in terrain_resolution:
			surface_tool.add_index(vert_idx)
			surface_tool.add_index(vert_idx + 1)
			surface_tool.add_index(vert_idx + terrain_resolution + 1)

			surface_tool.add_index(vert_idx + terrain_resolution + 1)
			surface_tool.add_index(vert_idx + 1)
			surface_tool.add_index(vert_idx + terrain_resolution + 2)
